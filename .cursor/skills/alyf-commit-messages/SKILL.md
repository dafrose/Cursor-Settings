...
# ALYF commit messages

## Scope

Use when drafting or revising git commit messages for ALYF repos that run
commitlint with `@commitlint/config-conventional`. Covers subject/body format, line length,
and optional Jira key placement via AskQuestion. Not for CI workflow setup (see
**alyf-frappe-app-ci**) or PR bodies (see **github-mcp-pull-requests**).
...
- [`alyf-frappe-app-ci`](alyf-frappe-app-ci/SKILL.md) — commitlint hook and `commitlint.config.mjs` in the app baseline
