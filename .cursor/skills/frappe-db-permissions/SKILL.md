...
Use when choosing ORM/db APIs (`get_list`, `get_all`, `get_doc`, `db.set_value`, …)
and whether permission checks apply by default. Complements the permission *model* in
**frappe-permissions** with an API matrix. Not for Role / DocPerm design alone.
...
Decorator options, HTTP verbs, guest hardening, and handler placement: see
**frappe-whitelist**. Inside handlers:
...
- [`frappe-permissions`](frappe-permissions/SKILL.md) — permission layers, `has_permission` vs `check_permission`
- [`frappe-whitelist`](frappe-whitelist/SKILL.md) — `@frappe.whitelist` decorator and API entry points
