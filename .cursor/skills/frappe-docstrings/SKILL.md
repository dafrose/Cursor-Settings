...
Use when writing or editing Python docstrings in Frappe apps—choosing Google-style
vs reST to match surrounding modules, or documenting hooks and whitelisted APIs.
...
- **Google-style:** `frappe/utils/logger.py` (`get_logger`), `frappe/utils/data.py` (`get_timedelta`).
- **reST:** `frappe/model/document.py`, `frappe/database/database.py`.
