...
Use when implementing side effects on field changes—`has_value_changed`,
`get_doc_before_save`, insert vs update, and hook placement (`before_save` vs `on_update`).
...
- [`frappe-background-jobs`](frappe-background-jobs/SKILL.md) — enqueue from `on_update` without blocking saves
- [`frappe-testing`](frappe-testing/SKILL.md) — assert change detection across saves
