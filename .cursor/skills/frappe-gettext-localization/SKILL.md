...
Use when generating or fixing gettext catalogs (`main.pot`, `*.po`, `*.mo`) for Frappe
v15+ apps—`bench` locale commands and `hooks.py` metadata for POT/PO headers. Not for backport
merge conflict replay (see **frappe-backport-merge-conflicts**).
...
- `app_title` -> `Project-Id-Version` project name (e.g. `"My Custom App VERSION"`).
...
- `app_publisher = "Example Corp GmbH"`
...
- [`frappe-backport-merge-conflicts`](frappe-backport-merge-conflicts/SKILL.md) — locale conflicts on backport PRs
