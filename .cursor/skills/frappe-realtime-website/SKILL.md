...
- **frappe-notifications**: Desk **Notification** lifecycle, Jinja in email templates, **Communication** assertions, silent failures.

## Pitfalls

- **Guest**: never publish sensitive data without a user filter.
- **Duplicate boot processing**: always clear the cache flag or use an idempotent “high water mark” timestamp.
- **Desk vs website**: `web_include_js` affects **website** templates, not Desk `app_include_js`.
